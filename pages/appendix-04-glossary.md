...
| 单控, 多控, 群控    | 单位控制常用术语, 分别为: 控制固定的一个单位, 依次控制固定的多个单位, 依次控制全体单位                                                                                                     |
...
| ---               | ---               | ---                                                                                           |
| cell              | 内存元            | 常用于存储一些数字                                                                            |
| bank              | 内存库            | 与 cell 一样, 但能储存的数字更多                                                              |
| message           | 信息板            | 用于输出字符串, 常用来主动显示一些信息说明逻辑执行状态                                        |
| display           | 显示屏            | 逻辑使用 draw 和 drawflush 在显示屏上画图                                                     |
| processor         | 处理器            | 逻辑块本身, 可以用于禁用或读写其它逻辑变量                                                    |
| switch            | 开关              | 玩家点击可禁用或启用, 使用 sensor 的 @enabled 以获取, 用于快捷控制一些逻辑执行                |
| canvas            | 画板              | E星 的特殊建筑, 可以绘制几个固定颜色, 逻辑可类似内存一样读写                                  |
| sorter            | 分类器            | 用于控制物品, 或通过选择的物品表示通常在 0-16 的一个数字, 也可以禁用变成反向分类, 溢流门同理  |
